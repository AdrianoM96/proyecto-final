...
export const AddressForm = ({ countries, userStoredAddress = {} }: Props) => {
  ...
  const getCountryValue = (country: UserStoredAddress['country']): string => {
  ...
  const countryValue = getCountryValue(userStoredAddress?.country);
  const { handleSubmit, register, formState: { isValid, errors }, reset, setValue, trigger } = useForm<FormInputs>({
    mode: 'onChange',
    defaultValues: {
      ...(userStoredAddress as any),
      country: userStoredAddress ? countryValue : "",
  ...
  }, [userStoredAddress, setValue, trigger]);
  ...
  const onSubmit = async( data: FormInputs ) => {
    ...
    if ( rememberAddress ) {
      ...
      await setUserAddress(restAddress, user?._id || "null", cookies.token );

    } else {
     
      await deleteUserAddress(user?._id || "null", cookies.token);
    }
  ...
  return (
    <form onSubmit={ handleSubmit( onSubmit ) }  className="grid grid-cols-1 gap-2 sm:gap-5 sm:grid-cols-2">
      ...
      <div className="flex flex-col mb-2">
        <span>Teléfono</span>
        <input type="number" className="p-2 border rounded-md bg-gray-200" {...register('phone', {
